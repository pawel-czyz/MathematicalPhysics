...
% Till now we defined everything in terms of algebraic objects - e.g vectors are abstract things that can be added and multiplied by numbers. This is a very powerful approach, enabling one to see deep
% connections between various structures. We were focused on natural operations and constructions, that did not employ the (not-natural) isomorphism between an $n$-dimensional, abstract, vector spaces and $\F^n$.
% Sometimes it will be useful to do some calculations using $\F^n$ and extract algebraic or geometrical meaning from obtained tuple of numbers.
%
% \begin{exercise}
%   Let $f:V\to V$ be an automorphism of a finite dimensional vector space. Prove that if $e_1, e_2,\dots e_n$ is a basis for $V$, then $f(e_1), f(e_2),\dots,f(e_n)$ is also a basis.
% \end{exercise}
%
% \begin{definition}
%   Consider $\F^n$. We say that
% \end{definition}
%
% \begin{definition}
%   Let $V$ be a $n$-dimensional vector space and $e_1, e_2,\dots e_n$ be a
% \end{definition}
