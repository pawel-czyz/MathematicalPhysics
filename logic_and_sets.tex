...
\subsection{Some sets you already know - PRZESUNĄĆ DO ROZDZIAŁU O REAL ANALYSIS?}
As we assumed that real numbers form a set $\mathbb R$, we will construct a few subsets that are often
useful:
\begin{align*}
...
\begin{prob} Let $A_i\subseteq U,\, i\in I$ and 
	...
	Prove that:
	\begin{enumerate}
		\item if $k\in I$, then $k\cup \sigma=\sigma$
...
\begin{prob}
	In Mathsland there is $n\ge 2$ cities. Between each pair of them there is a one-way road. 
	\begin{enumerate}
...
\begin{prob}
	Here you can find the difference between finite and infinite sets - for finite sets a proper subset (a subset that is not the whole set)
	always have smaller number of elements. In the infinite case it is not true - prove that:
	\begin{enumerate}
...
\begin{prob}
	\textbf{Cantor's theorem} Let $A$ be a set and $f:A\to 2^A.$
	\begin{enumerate}
		\item Consider $X=\{a\in A : x\notin f(a)\}\in 2^A$. Is there $x\in A$ for which $f(x)=X?$
...
\begin{prob}
	...
	\begin{enumerate}
		\item (Tarski) Let $A$ be a set and $F:2^A\to 2^A$ has property: $F(X)\subseteq F(Y)$ if $X\subseteq Y$. Prove that $F(S)=S$, where:
